...
function BookingModal({ room, onClose }: { room: Room; onClose: () => void }) {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token) {
        Swal.fire("Harus login dulu", "Silakan login untuk booking kamar", "warning");
        setLoading(false);
        return;
      }
      ...
      Swal.fire({
        ...
        text: "Silakan lanjutkan pembayaran di halaman Pesanan.",
        icon: "success",
        ...
        <div className="p-6">
          ...
          <form className="space-y-4" onSubmit={handleBookingSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full bg-gradient-to-r from-amber-500 to-amber-600 text-white py-4 rounded-lg font-semibold text-lg shadow-lg hover:shadow-xl transition flex justify-center items-center ${
...
export default function KamarPage() {
  ...
  const handlePesanClick = (room: Room) => {
    const token = localStorage.getItem("token");
    if (!token) {
      Swal.fire({
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Tipe: {room.type}, Kapasitas: {room.capacity}, Ukuran:{" "}
                  {room.size}
                </p>
                <button
